...
/*   Updated: 2025/07/22 23:47:23 by nmikuka          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	run_pipex(t_mshell_data *mshell_struct)
{
	...
	while (i < mshell_struct->pipex->n_cmds)
	{
		...
		{
			handle_redirections(mshell_struct->pipex->commands[i].redirections);
...
void	run_cmd(t_command command, t_mshell_data *mshell_struct, int fd[2])
{
	char	*cmd;
	int		err_code;
	int		builtin_status;
